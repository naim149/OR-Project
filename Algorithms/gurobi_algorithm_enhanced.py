...
class GurobiOptimizationEnhanced:
    def __init__(self):
        self.name = "Gurobi Optimization Enhanced"
    ...
    def optimize_allocation(self, optimization_instance: OptimizationInstance, initial_guesses: Optional[Dict[str, np.ndarray]] = None) -> Dict:
        ...
        model.addConstr(gp.quicksum((U[i, t]/(num_students*num_time_slots)) for t in range(num_time_slots) for i in range(num_students)) == A, name=f"average_of_usage")
