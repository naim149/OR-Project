...
class GurobiOptimization:
    ...
    def optimize_allocation(self, optimization_instance: OptimizationInstance) -> Dict:
        ...
        B = model.addVars(num_students, num_time_slots + 1, ub=100, vtype=GRB.CONTINUOUS, name="B")
        ...
        for i in range(num_students):
            for t in range(num_time_slots + 1):
                model.addConstr(B[i, t] <= 100, name=f"max_battery_{i}_{t}")
                model.addConstr(B[i, t] >= 0, name=f"min_battery_{i}_{t}")
